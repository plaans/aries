...
fn signed_var_of_same_sign(var: VarRef, svar: SignedVar) -> SignedVar {
    match svar.is_plus() {
        ...
        false => { debug_assert!(svar.is_minus()); SignedVar::minus(var) },
    }
}
...
impl MapSolver {
    pub fn new(literals: impl IntoIterator<Item = Lit>) -> Self {
        ...
        let solve_fn: Box<dyn FnMut(&mut Solver<_>) -> Result<Option<Arc<SavedAssignment>>, Exit>> = match solving_mode {
            ...
            "OPTIMIZE_HIGH" => {
                ...
                Box::new(
                    move |_s: &mut Solver<_>| {
                        Ok((*_s).maximize(obj)?.map(|(_, doms)| doms))
                    }
                )
            }
            ...
            "OPTIMIZE_LOW" => {
                ...
                Box::new(
                    move |_s: &mut Solver<_>| {
                        Ok((*_s).minimize(obj)?.map(|(_, doms)| doms))
                    }
                )
            }
            ...
            "PREFERRED_VALUES_HIGH" => {
                ...
                Box::new(move |_s: &mut Solver<_>| (*_s).solve())
            }
