...
cur.execute(
    """
    CREATE TABLE IF NOT EXISTS jobshop (
...
folder = Path("/home/roland/tyr/logs/aries/socs2025-jobshop/")
for instance in folder.iterdir():
    ...
    base_cmd = (
        f"cargo run --profile ci --bin up-server -- -l debug solve {pb.as_posix()}"
    )
        ...
        for use_bp in [True, False]:
            ...
            with Popen(cmd, shell=True, stdout=PIPE) as proc:
                for line in proc.stdout:
                    ...
                    if match := re.match(
                        r"^(.+)\s*:\s*\(t_(\d+)_(\d+)\)\s*\[(.+)\]", line
                    ):
                        num_jobs = max(num_jobs, int(match.group(2)) + 1)
                        num_machines = max(num_machines, int(match.group(3)) + 1)
                        quality = max(
                            quality, float(match.group(1)) + float(match.group(4))
                        )

            print(
                f"  {num_jobs} {num_machines} {db_size} {cp_size} {num_bp} {computation_time} {quality}"
            )
            cur.execute(
                """
                INSERT INTO jobshop
                (instance_id, factorized, use_bp, num_jobs, num_machines, db_size, cp_size, num_bp, computation_time, quality)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    instance_id,
