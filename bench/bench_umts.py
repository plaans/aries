...
cur.execute(
    """
    CREATE TABLE IF NOT EXISTS umts (
...
folder = Path("/home/roland/tyr/logs/aries/socs2025-umts/")
for instance in folder.iterdir():
    ...
    base_cmd = f"ARIES_UP_ASSUME_REALS_ARE_INTS=true cargo run --profile ci --bin up-server -- -l debug solve {pb.as_posix()}"
            ...
            with Popen(cmd, shell=True, stdout=PIPE) as proc:
                for line in proc.stdout:
                    ...
                    if match := re.match(r"^(.+)\s*:\s*\(.+\)\s*\[(.+)\]", line):
                        quality = max(quality, float(match.group(1)) + float(match.group(2)))
                    if "(bs " in line:
                        num_goals += 1

            print(f"  {num_goals} {db_size} {cp_size} {num_bp} {computation_time} {quality}")
            cur.execute(
                """
                INSERT INTO umts
                (instance_id, factorized, use_bp, num_goals, db_size, cp_size, num_bp, computation_time, quality)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (instance_id, factorized, use_bp, num_goals, db_size, cp_size, num_bp, computation_time, quality),
            )
