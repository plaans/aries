...
A variable in aries core is represented by the `Var` type.
A `Var` type is the ID of a variable.
...
Among the possible instances of a `Var`, the `Var::ZERO` constant is a reserved value that is always equal to `0`.
...
IT is possible to manually create variables by using `Var::from_u32(i)` where `i` an unsigned integer on 32 bits (`u32`). This is typically non what you want to do but allows:
...
The first variable is reserved as the `Var::ZERO` value.

```rust
assert_eq!(Var::from_u32(0), Var::ZERO);
``` 
