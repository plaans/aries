...
use crate::ext::lprelax::encoder::encode::{encode_problem_ground, encode_problem_lifted};
use crate::ext::lprelax::encoder::groundings::{
...
use crate::ext::lprelax::encoder::problem::LpRelaxProblem;
use crate::{
...
impl LpRelaxEncoder {
    ...
    pub fn post_ground_source(&mut self, source: Source, source_grounding: SourceGrounding, ctx: &SchedEncoder) {
        ...
        let source_grounding_id = self.sources_ground.post_ground_source(source, source_grounding.clone());
    ...
    pub fn run_new_brutal_grounder(&self, ctx: &SchedEncoder) -> HashMap<Option<TaskId>, Vec<SourceGrounding>> {
        ...
        for (source, _) in self.iter_sources() {
            res.insert(
                source,
    ...
    pub fn encode(&mut self, ctx: &SchedEncoder, doms: Option<&crate::Domains>) -> LpRelaxProblem {
