use aries::{core::views::Term, prelude::*};
...
impl SimpleDatalogGrounderInner {
    ...
    fn run(self) -> HashMap<Source, Vec<SourceGrounding>> {
        ...
        for (predicate_id, predicate_index) in self.predicates {
            ...
            let rows = match var_tables[predicate_index].extract() {
                aries_datalog::VarTableExtract::NonNullary(table) => table
                    .rows()
                    .map(|row| SourceGrounding::from(Vec::from_iter(row.iter().map(|&u| self.cst_of_datalog_sym[u]))))
                    .collect(),
                aries_datalog::VarTableExtract::Nullary(true) => vec![SourceGrounding::from(vec![])],
                aries_datalog::VarTableExtract::Nullary(false) => vec![],
            };
...
fn collect_condition_datalog_terms(
    ...
    Ok(Vec::from_iter(terms.into_iter().map(|term| {
        if term.is_cst() {
            SimpleDatalogGrounderTerm::Cst(term.cst())
        } else {
...
fn collect_effect_datalog_terms(
    ...
    let negative = is_effect_boolean(eff, ctx).unwrap() && eff_value_term.is_cst() && eff_value_term.cst() == 0;
...
fn is_effect_boolean_negative(eff: &crate::Effect, ctx: &SchedEncoder) -> Result<bool, ()> {
    ...
    Ok(
        !eff_value_param.is_sym_typed() && eff_value_term.is_cst() && eff_value_term.cst() == 0, // TODO: change "!is_sym_typed" to "is_boolean_typed"
    )
}
fn is_effect_boolean_positive(eff: &crate::Effect, ctx: &SchedEncoder) -> Result<bool, ()> {
    ...
    let eff_value_param = ctx.sched.fluents.get_return(&eff.state_var.fluent).unwrap();
    Ok(
        !eff_value_param.is_sym_typed() && eff_value_term.is_cst() && eff_value_term.cst() == 1, // TODO: change "!is_sym_typed" to "is_boolean_typed"
    )
}
