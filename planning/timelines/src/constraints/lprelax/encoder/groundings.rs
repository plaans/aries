...
use crate::IntTerm;
use crate::analysis::{Source, grounding::ParametersAssignment};
use crate::constraints::lprelax::encoder::Supports;
use crate::constraints::lprelax::transitions::TransitionId;
use crate::constraints::lprelax::transitions::ground::*;
...
pub(crate) struct SourcesGroundingsInfo {
    ...
    groundings_rev: Vec<(Source, ParametersAssignment)>,

    #[cfg(debug_assertions)]
    groundings: HashMap<(Source, ParametersAssignment), SourceGroundingId>,
}
impl SourcesGroundingsInfo {
    ...
    pub fn post_ground_source(&mut self, source: Source, source_grounding: &ParametersAssignment) -> SourceGroundingId {
        #[cfg(debug_assertions)]
...
mod tests {
    ...
    fn test_sources_groundings_addition() {
        ...
        sources_groundings.post_ground_source(
            Some(TaskId::from_int(1)),
            &ParametersAssignment::from(vec![1000, 50, 40]),
        );
        sources_groundings.post_ground_source(
            Some(TaskId::from_int(1)),
            &ParametersAssignment::from(vec![1000, 50, 30]),
        );

        sources_groundings.post_ground_source(
            Some(TaskId::from_int(0)),
            &ParametersAssignment::from(vec![2, 10, 400, 300]),
        );

        sources_groundings.post_ground_source(None, &ParametersAssignment::from(vec![]));

        sources_groundings.post_ground_source(
            Some(TaskId::from_int(0)),
            &ParametersAssignment::from(vec![2, 10, 401, 300]),
        );
        sources_groundings.post_ground_source(
            Some(TaskId::from_int(0)),
            &ParametersAssignment::from(vec![2, 10, 400, 301]),
        );
        sources_groundings.post_ground_source(
            Some(TaskId::from_int(0)),
            &ParametersAssignment::from(vec![2, 10, 401, 301]),
        );
    ...
    fn test_sources_groundings_panic_on_dupe() {
        ...
        let result = catch_unwind_silent(|| {
            ...
            sources_groundings
                .post_ground_source(Some(TaskId::from_int(0)), &ParametersAssignment::from(vec![1, 1, 1]));
            sources_groundings
                .post_ground_source(Some(TaskId::from_int(0)), &ParametersAssignment::from(vec![1, 1, 1]));
        });
