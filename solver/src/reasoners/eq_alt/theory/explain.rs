...
impl AltEqTheory {
    ...
    pub fn neq_cycle_explanation_path(&self, constraint_id: ConstraintId, model: &DomainsSnapshot) -> Vec<Edge> {
        ...
        let source_id = self.enabled_graph.get_id(&constraint.b).unwrap();
        let target_id = self.enabled_graph.get_id(&constraint.a).unwrap();

        // Transform the enabled graph to get a snapshot of it just before the propagation
        let graph = self.enabled_graph.outgoing.filter(|_, e| model.entails(e.active));
        ...
        .expect("Unable to explain Neq cycle propagation")
    }
    ...
    pub fn eq_explanation_path(&self, literal: Lit, model: &DomainsSnapshot<'_>) -> Vec<Edge> {
        let source_id = self.enabled_graph.get_id(&Node::Var(literal.variable())).unwrap();
    ...
    pub fn deactivation_explanation_path(
        &self,
        ...
        let (graph, source_id, other_node) = if start {
            let id = self.enabled_graph.get_id(&constraint.b).unwrap();
            (&self.enabled_graph.outgoing, id, constraint.a)
        } else {
            let id = self.enabled_graph.get_id(&constraint.a).unwrap();
            (&self.enabled_graph.incoming, id, constraint.b)
        };
        ...
        let cause = graph
            ...
            .skip(1)
            .find(|EqNode(id, r)| {
                let node = self.enabled_graph.get_node(*id);
                let Some(total_relation) = *r + constraint.relation else {
                    return false;
                };
                // If Eq or Neq propagation fails
                total_relation == EqRelation::Eq && { !other_node.can_be_eq(&node, model) }
                    || total_relation == EqRelation::Neq && { !other_node.can_be_neq(&node, model) }
            })
            .expect("Unable to explain Deactivation propagation.");
    ...
    pub fn explain_from_path(
        ...
        if cause == DomEq || cause == DomNeq {
            let origin = self.enabled_graph.get_node(
                path.first()
        ...
        if let EdgeDeactivation(id, start) = cause {
            // TODO: Be smarter about which bounds to push
            let constraint = self.constraint_store.get_constraint(id);

            if start {
                if let Node::Var(v) = constraint.a {
                    out_explanation.push(v.leq(model.ub(v)));
                    out_explanation.push(v.geq(model.lb(v)));
                }
            } else if let Node::Var(v) = constraint.b {
                out_explanation.push(v.leq(model.ub(v)));
                out_explanation.push(v.geq(model.lb(v)));
            }
            let origin = self.enabled_graph.get_node(
