...
pub trait IntExpr<Ctx: Store> {
    ...
    fn linearize(&self, scope: impl Into<Conjunction>, ctx: &mut Ctx) -> LinearSum {
        self.reify(scope, ctx).into()
    }

    /// Enforce that, whenever `variable` is defined, it has the same value as the expression.
    fn enforce_eq(&self, variable: IAtom, ctx: &mut Ctx) {
    ...
    fn enforce_eq_if(&self, implicant: Lit, variable: IAtom, ctx: &mut Ctx);
}
