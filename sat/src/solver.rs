...
use aries_collections::{Next, Range};
use itertools::Itertools;
use std::collections::{HashSet, VecDeque};
use std::fmt::{Debug, Formatter};
use std::num::NonZeroU32;
use std::ops::Index;
use SearchStatus::{Conflict, Consistent, Pending, Solution, Unsolvable};
...
// TODO: should be made independent of solver and moved to top-level
...
#[derive(Debug)]
pub struct Model<'a>(&'a solver::Solver);

impl<'a> Model<'a> {
    fn get_value(&self, var: BVar) -> bool {
        debug_assert_eq!(self.0.search_state.status, solver::SearchStatus::Solution);
