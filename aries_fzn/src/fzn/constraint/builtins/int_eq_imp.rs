...
use crate::aries::constraint::EqHalf;
use crate::fzn::Fzn;
...
/// Half reified integer equality constraint.
///
/// ```flatzinc
/// constraint int_eq_imp(x,y,r);
/// % r -> x = y
/// ```
#[derive(Clone, Debug)]
pub struct IntEqImp {
...
impl TryFrom<Constraint> for IntEqImp {
    type Error = anyhow::Error;

    fn try_from(value: Constraint) -> Result<Self, Self::Error> {
        match value {
            Constraint::IntEqImp(c) => Ok(c),
            _ => anyhow::bail!("unable to downcast to {}", Self::NAME),
        }
    }
}

impl From<IntEqImp> for Constraint {
    fn from(value: IntEqImp) -> Self {
        Self::IntEqImp(value)
    }
}

impl Encode for IntEqImp {
    fn encode(
        &self,
        ...
        Box::new(EqHalf::new(IVar::new(*a), IVar::new(*b), BVar::new(*r)))
    }
}
