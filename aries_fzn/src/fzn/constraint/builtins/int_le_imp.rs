...
use crate::aries::constraint::LeHalf;
use crate::fzn::Fzn;
...
impl TryFrom<Constraint> for IntLeImp {
    type Error = anyhow::Error;

    fn try_from(value: Constraint) -> Result<Self, Self::Error> {
        match value {
            Constraint::IntLeImp(c) => Ok(c),
            _ => anyhow::bail!("unable to downcast to {}", Self::NAME),
        }
    }
}

impl From<IntLeImp> for Constraint {
    fn from(value: IntLeImp) -> Self {
        Self::IntLeImp(value)
    }
}

impl Encode for IntLeImp {
    fn encode(
        &self,
        ...
        Box::new(LeHalf::new(IVar::new(*a), IVar::new(*b), BVar::new(*r)))
    }
}
