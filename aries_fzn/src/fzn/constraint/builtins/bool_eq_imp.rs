...
/// Half reified boolean equality constraint.
///
/// ```flatzinc
...
impl BoolEqImp {
    pub const NAME: &str = "bool_eq_imp";
...
impl TryFrom<Constraint> for BoolEqImp {
    type Error = anyhow::Error;

    fn try_from(value: Constraint) -> Result<Self, Self::Error> {
        match value {
            Constraint::BoolEqImp(c) => Ok(c),
            _ => anyhow::bail!("unable to downcast to {}", Self::NAME),
        }
    }
}

impl From<BoolEqImp> for Constraint {
    fn from(value: BoolEqImp) -> Self {
        Self::BoolEqImp(value)
    }
}
