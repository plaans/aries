...
use crate::planning::classical::state::{World, Lit, Operator, State, Operators};
use crate::planning::utils::enumerate;
use streaming_iterator::StreamingIterator;
use crate::planning::ref_store::Ref;
use crate::planning::chronicles::{Type, Time, Ctx, Holed};
use std::hash::Hash;
use std::fmt::Display;
...
fn sv_to_lit<T,I,A: Ref>(variable: &[A], value: &A, world: &World<T,I>, ctx: &Ctx<T,I,A>) -> Result<Lit, String> {
    let sv: Result<Vec<SymId>, _> =
        variable.iter()
            .map(|var| ctx.sym_value_of(*var).ok_or("Not a symbolic value"))
            .collect();
    let sv = sv?;
    let sv_id = world.sv_id(&sv).ok_or("No state variable identifed (maybe due to a typing error")?;
...
fn holed_sv_to_pred<T,I,A: Ref>(variable: &[Holed<A>], value: &Holed<A>, to_new_param: &HashMap<usize,usize>, ctx: &Ctx<T,I,A>) -> Result<ParameterizedPred, String> {
...
pub fn from_chronicles<T,I,A: Ref>(chronicles: &crate::planning::chronicles::Problem<T,I,A>)
    -> Result<LiftedProblem<T,I>, String>  where
...
I: Clone + Eq + Hash + Display
{
    ...
    for instance in &chronicles.chronicles {
        ...
        check(ch.prez == ctx.tautology(), "A chronicle instance is optional")?;
        for eff in &ch.effects {
            check(eff.effective_start() == eff.transition_start(), "Non instantaneous effect")?;
            check(*eff.effective_start() == ctx.origin(), "Effect not at start in initial chronicle")?;
        ...
        for cond in &ch.conditions {
            ...
            check(*cond.start() == ctx.horizon(), "Non final condition can not be interpreted as goal")?;
    ...
    for template in &chronicles.templates {
        ...
        check(template.chronicle.start.partial_cmp(&global_start).is_none(), "action start is not free")?;
        check(template.chronicle.start.partial_cmp(&global_end).is_none(), "action start is not free")?;
        check(template.chronicle.start < template.chronicle.end, "More than one free timepoint in the action.")?;
        ...
        while let Some(x) = iter.next() {
            match x {
                ...
                _ => return Err(format!("Expected an action parameter but got an expression"))
            }
        }
        ...
        for cond in &template.chronicle.conditions {
            ...
            check(*cond.start() == template.chronicle.start, "Non final condition can not be interpreted as goal")?;
        ...
        for eff in &template.chronicle.effects {
            check(eff.transition_start() == &template.chronicle.start, "Effect does not start condition with action's start")?;
            check(eff.effective_start() == &template.chronicle.end, "Effect is not active at action's end")?;
...
fn ground_action_schema<T,I>(schema: &ActionSchema, desc: &World<T,I>) -> Result<Vec<Operator>, String> {
    let mut res = Vec::new();
